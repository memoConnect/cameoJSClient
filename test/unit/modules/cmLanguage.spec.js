define(['app', 'jquery', 'angular-mocks'], function (app, $) {
    'use strict';
    ...
    describe("cmLanguage", function() {
        ...
        xdescribe("setup", function(){
            ...
            it('should find and load correctly named and json formatted files for all supported languages within 5 second.', function() {
                ...
                runs(function(){
                    app.cameo.supported_languages.forEach(function(lang_key){
                        var file = 'app/'+app.cameo.path_to_languages+'/lang-'+lang_key+'.json'

                        jasmine.getFixtures().fixturesPath = 'app/'+app.cameo.path_to_languages
                        var f = readFixtures('lang-'+lang_key+'.json')

                        language_tables[lang_key] = JSON.parse(f);
                        count--;
